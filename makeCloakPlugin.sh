...
DEPS=$(pwd)/main/src/main/cpp/.deps
SRC_DIR=$(pwd)/main/src/main/cpp

try mkdir -p $DEPS $SRC_DIR/pluginjni/armeabi-v7a $SRC_DIR/pluginjni/x86 $SRC_DIR/pluginjni/arm64-v8a $SRC_DIR/pluginjni/x86_64

try cd $DEPS
echo "Getting Cloak source code"
rm -rf Cloak
try git clone https://github.com/amnezia-vpn/Cloak
cd Cloak
try go get ./...
